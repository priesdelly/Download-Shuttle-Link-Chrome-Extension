...
### ทำไม `onCreated` ต้องเป็น `async`?

ต้อง await message round-trip ไปยัง content script เพื่อขอสถานะปุ่ม Alt:
...
ถ้าเป็น sync listener จะต้องตัดสินใจดักจับโดยไม่รู้ว่าผู้ใช้กด Alt อยู่ไหม
